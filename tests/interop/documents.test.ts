...
const toDocumentFromLlama = (doc: LlamaDocument): Document => ({
  id: doc.id_,
  text: doc.text,
  metadata: doc.metadata,
});
...
describe("Interop documents", () => {
  it("maps LangChain Document to Document", () => {
    ...
    const adapted = toDocumentFromLangChain(doc);
  ...
  it("maps LlamaIndex Document to Document", () => {
    const doc = new LlamaDocument({
      text: "Hello",
      metadata: { source: "llama" },
    });

    const adapted = toDocumentFromLlama(doc);
