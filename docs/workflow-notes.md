...
- Diagnostics severity over strictness: default mode is permissive + diagnosable; "strict" treats missing requirements/incompatible plugins as build-time failures.
...
6) Resume: if a run returns `needsHuman`, call `resume` with the token.
...
- `.resume(token, humanInput?)` -> outcome union (recipe-provided, only if needed)
...
These names (`createPipelineExtension`, `makeLifecycleStage`) refer to existing **core** pipeline constructs. They are mentioned here only to anchor how plugins/recipes map to what already exists. They are *not* part of the user-facing Workflow API.
...
type Outcome<TArtefact> =
  ...
  | { status: "needsHuman"; token: ResumeToken; artefact: TArtefact; trace: TraceEvent[]; diagnostics: Diagnostic[] }
...
type WorkflowRuntime<TRunInput, TArtefact> = {
  ...
  resume?: (token: ResumeToken, humanInput?: unknown, runtime?: Runtime) => MaybePromise<Outcome<TArtefact>>;
...
type ResumableWorkflowRuntime<N extends RecipeName> = WorkflowRuntime<RunInputOf<N>, ArtefactOf<N>> & {
  resume: (token: ResumeToken, humanInput: HumanInputOf<N>, runtime?: Runtime) => MaybePromise<Outcome<ArtefactOf<N>>>;
};
