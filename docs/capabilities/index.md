...
A capability is a stable contract for one kind of work. The contract describes portable inputs and outputs. A live implementation enters later through explicit host composition, a capability adapter, or the runtime integration selected by your application.

This separation lets application code depend on what a component does without depending on the provider or framework that performs it.
...
Retrieval, indexing, storage, memory, and media are independent capability fronts at `/retrieval`, `/indexing`, `/storage`, `/memory`, and `/media`. `/model` owns provider-neutral model requests, responses, profiles, and content contracts. None of these fronts selects a hosted service, executes an agent loop, or exposes provider-native clients.
