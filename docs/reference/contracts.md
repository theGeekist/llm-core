...
Live handles, event sinks, provider clients, and generic extension escape hatches remain outside the context.
...
The checked JSON Schema is generated from the TypeScript contract source. A schema reference binds consumers to the exact serialized schema bytes.
...
Physical paths, signed URLs, buckets, credentials, and live resolver objects do not enter portable content. Byte access occurs through an explicit live port.
...
The package publishes its generated contract schema and verifies the committed document during typecheck and CI. Use [Package exports](/reference/package-exports) for the module surface and [API by subpath](/reference/api) for the owning capability of each higher-level contract.
