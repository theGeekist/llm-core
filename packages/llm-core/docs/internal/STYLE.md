...
- links into package engineering documents such as
  `packages/**/docs/final-architecture/**`;
...
Treat every README-sized example as a language test. Agent and workflow guides
must name the required runtime-integration boundary instead of inventing a
kernel-owned executor for convenience.

## Authoritative mental model

Definitions describe portable agent intent. Qualified runtime integrations
prepare and execute agent runs. Workflow plans carry portable intent into a
selected native runtime. Interactions project canonical events into
deterministic state. Capabilities define ports, and qualified adapters connect
external systems to them.

| Noun                    | Meaning                                                                                                |
| ----------------------- | ------------------------------------------------------------------------------------------------------ |
| `AgentDefinition`       | Portable agent intent: identity, version, instructions, effect requirement, metadata, and skills.      |
| `AgentRunner`           | Integration port that prepares a definition and starts a run. Resume is optional and capability-gated. |
| `AgentRun`              | Live handle exposing `AgentEvent` values, typed controls, and one terminal result.                     |
| `AgentResult`           | Terminal agent result: `completed`, `failed`, `denied`, or `cancelled`.                                |
| `WorkflowExecutionPlan` | Portable workflow intent projected for an explicit runtime target.                                     |
| Native workflow         | Runtime-owned graph or durable workflow; it is not a portable kernel execution object.                 |
| `ToolExecutionEvent`    | Redacted evidence for controlled tool execution. It is not an agent event or a trace.                  |
...
public llm-core type or package export. Do not revive v1 runnable workflow or
local-runner types in examples or prose.
