...
This ledger tracks published documentation against the current 29-entrypoint,
ESM-only package surface and ADR-016 runtime-ownership boundary.

## Authoritative package model

- The root exports the smallest common portable contract journey.
- Independent capability subpaths cover contracts, model, tools, control,
  evidence, state, context, artifacts, and evaluation.
- `/agent` and `/agent/runtime` expose portable agent intent, normalized run
  facts, and the `AgentRunner` integration port. They do not select a runner.
- `/workflow` exposes portable workflow intent. Native runtimes own execution,
  pause, retry, checkpoint, and resume semantics.
...
| Orchestration              | workflow intent and controlled execution                                                             | Separate adapter-owned workflow execution from kernel-owned effect controls.   |
