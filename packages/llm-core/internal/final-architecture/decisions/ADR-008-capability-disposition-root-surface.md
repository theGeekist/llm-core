...
Affected tasks: core-knowledge, core-conversations, core-media-schemas-skills, core-capability-bindings, core-convergence
Supersedes: none

## Context

Core convergence found supported retrieval, indexing, storage, conversation
...
- Migrate knowledge/retrieval/indexing, storage/conversation and
  media/model-schema/agent-skill contracts in three disjoint core-stage spokes.
- Build typed capability bindings only after those fronts exist.
- Keep legacy implementations and tests unchanged as evidence until core-convergence
...
- The root is the minimal orchestration surface. Its sole runtime value is
  ...
  subpath.
- core-convergence publishes version `2.0.0`, validates runtime and declaration targets
...
The core stage gains four bounded tasks before convergence, but no supported surface is
silently discarded and core-convergence no longer invents domain semantics. Root imports
