...
import { combineTriStates, maybeReduceTriState } from "#shared/tri-state";
...
const emitTraceEvents = (handler: BaseCallbackHandler, events: AdapterTraceEvent[]) =>
  maybeReduceTriState(bindFirst(emitTracePair, handler), events);
