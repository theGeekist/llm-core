...
describe("Maybe utilities", () => {
  it("maybeMap maps a MaybePromise value", () => {
    expect(maybeMap(addOne, 2)).toBe(3);
  });

  it("maybeChain chains a MaybePromise value", () => {
  ...
  it("maybeAll preserves synchronous execution", () => {
    const values = [1, 2, 3];
  ...
  it("maybeReduce preserves synchronous reductions", () => {
    const result = maybeReduce(add, 0, [1, 2, 3]);

    expect(isPromiseLike(result)).toBe(false);
    expect(result).toBe(6);
  });

  it("maybeReduce waits for a deferred reduction before starting the next", async () => {
    let resolveFirst: (value: number) => void = (_value: number) => undefined;
    const seen: number[] = [];
    const first = new Promise<number>((resolve) => {
      resolveFirst = resolve;
    });
    const result = maybeReduce(
      (total, value) => {
        seen.push(value);
        return value === 1 ? first : total + value;
      },
      0,
      [1, 2],
    );
  ...
  it("maybeMapArray maps a MaybePromise array", () => {
