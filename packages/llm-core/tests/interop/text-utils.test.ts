...
const toTextSplitterFromLangChain = (splitter: RecursiveCharacterTextSplitter): TextSplitter => ({
  split: ({ text }) => splitter.splitText(text),
  splitBatch: ({ texts }) => maybeAll(texts.map((text) => splitter.splitText(text))),
...
const toTextSplitterFromLlama = (splitter: SentenceSplitter): TextSplitter => ({
  split: ({ text }) => splitter.splitText(text),
  splitBatch: ({ texts }) => texts.map((text) => splitter.splitText(text)),
  splitWithMetadata: ({ text }) => splitter.splitText(text).map((chunk) => ({ text: chunk })),
});
...
describe("Interop text utilities", () => {
  it("maps LangChain text splitter to TextSplitter", async () => {
    ...
    await expect(adapter.split({ text: TEXT_SAMPLE })).resolves.toBeArray();
    await expect(adapter.splitBatch?.({ texts: TEXT_BATCH })).resolves.toBeArray();
    await expect(adapter.splitWithMetadata?.({ text: TEXT_SAMPLE })).resolves.toBeArray();
  });
  ...
  it("maps LlamaIndex text splitter to TextSplitter", () => {
    ...
    expect(adapter.split({ text: TEXT_SAMPLE })).toBeArray();
    expect(adapter.splitBatch?.({ texts: TEXT_BATCH })).toBeArray();
    expect(adapter.splitWithMetadata?.({ text: TEXT_SAMPLE })).toBeArray();
  });
