...
describe("qualified storage adapters", () => {
  test("maps a truthful host cache contract and default TTL", async () => {
    ...
    expect(await cache.get({ context, key: "key" })).toEqual(value);
    ...
    for (const defaultTtl of [
      ...
      expect(() =>
        configured.set({
          context,
          key: "configured",
          value: jsonStorageValue({ safe: true }),
        }),
      ).toThrow();
    }
  ...
  test("maps LangChain cache and key-value stores", async () => {
    ...
    await expect(keyValue.setMany({ context, entries: [["pref:key", value]] })).resolves.toBe(true);
    await expect(keyValue.getMany({ context, keys: ["pref:key"] })).resolves.toEqual([value]);
    await expect(keyValue.list({ context, prefix: "pref" })).resolves.toEqual(["pref:key"]);
    await expect(keyValue.deleteMany({ context, keys: ["pref:key"] })).resolves.toBe(true);
    ...
    await expect(cache.get({ context, key: "cache" })).resolves.toEqual(value);
  });
  ...
  test("round-trips exact values through installed LlamaIndex stores", async () => {
    ...
    await expect(keyValue.getMany({ context, keys: ["doc-1", "doc-2", "doc-3"] })).resolves.toEqual(
      values,
    );
    await expect(keyValue.list({ context, prefix: "doc" })).resolves.toEqual([
      "doc-1",
      "doc-2",
      "doc-3",
    ]);
    ...
    await expect(keyValue.getMany({ context, keys: ["foreign"] })).resolves.toEqual([null]);
    ...
    await expect(keyValue.deleteMany({ context, keys: ["doc-1"] })).resolves.toBe(true);
    ...
    await expect(documentValues.getMany({ context, keys: ["node-1", "node-2"] })).resolves.toEqual([
    ...
    await expect(documentValues.deleteMany({ context, keys: ["node-1", "node-2"] })).resolves.toBe(
      true,
    );
  });
});
